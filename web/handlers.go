...
func (s *Server) handleSetConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := SetConfig(s.db, config.Key, config.Value, "用户设置"); err != nil {
...
func (s *Server) handleResetConfig(w http.ResponseWriter, r *http.Request) {
	// 这里可以实现重置配置到默认值的逻辑
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"status": "success", "message": "配置已重置"})
}
...
func (s *Server) handleDeleteTransaction(w http.ResponseWriter, r *http.Request) {
	...
	// 这里可以实现删除交易记录逻辑
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"status": "success", "message": "交易记录已删除"})
}
...
func (s *Server) handleUpdateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	// 这里可以实现更新交易记录逻辑
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"status": "success", "message": "交易记录已更新"})
}
